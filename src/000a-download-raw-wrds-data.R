...
# Download historical gics codes -----------------------------------------------
hgic <- comp.hgic  |> 
  collect()
...
# these years as leads or lags
raw_funda <- raw_funda |>
  #join raw_funda with a list of unique gvkey-calyears
  semi_join(raw_funda |>
              group_by(gvkey,calyear) |>
              count() |>
              ungroup() |>
...
#make a list of header gics codes
header_gics <- raw_funda |>
...
#take the historical gic if avail, otherwise take the current gic
funda_best_gic <- raw_funda |> 
left_join(header_gics |> select(gvkey,datadate,gindh=gind)) |> 
  mutate(best_gic = if_else(is.na(gindh),gind,gindh))
#same nobs as raw_funda so join is ok.

#Filter the dataset to match the paper
sample_ids <- read_csv("sample-id/sample_ids.csv")

compustat_sample <- funda_best_gic |>
